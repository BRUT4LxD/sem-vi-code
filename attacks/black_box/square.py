...
class Square(Attack):
    ...
    Arguments:
        ...
        n_queries (int): max number of queries (each restart). (Default: 1000)
    ...
    Examples::
        >>> attack = torchattacks.Square(model, model, norm='Linf', eps=8/255, n_queries=1000, n_restarts=1, eps=None, p_init=.8, seed=0, verbose=False, targeted=False, loss='margin', resc_schedule=True)
        >>> adv_images = attack(images, labels)

    """

    def __init__(self, model, norm='Linf', eps=8/255, n_queries=1000, n_restarts=1,
    ...
    def eta_rectangles(self, x, y):
        ...
        for counter in range(0, max(x_c, y_c)):
            delta[max(counter2[0], 0):min(counter2[0] + (2*counter + 1), x), max(0, counter2[1]):min(counter2[1] + (2*counter + 1), y)] += 1.0/(torch.Tensor([counter + 1]).view(1, 1).to(self.device) ** 2)  # nopep8
            counter2[0] -= 1
            counter2[1] -= 1
