...
class APGDT(Attack):
    ...
    def attack_single_run(self, x_in, y_in):
        ...
        if self.verbose:
            print('parameters: ', self.steps, self.steps_2, self.steps_min, self.size_decr)  # nopep8
        ...
        for i in range(self.steps):
            ...
            with torch.no_grad():
                ...
                if self.norm == 'Linf':
                    ...
                    x_adv_1 = x_adv + step_size[0] * grad / ((grad ** 2).sum(dim=(1, 2, 3), keepdim=True).sqrt() + 1e-12)  # nopep8
            ...
            acc_steps[i + 1] = acc + 0
            x_best_adv[(pred == 0).nonzero().squeeze()] = x_adv[(
                pred == 0).nonzero().squeeze()] + 0.
            if self.verbose:
